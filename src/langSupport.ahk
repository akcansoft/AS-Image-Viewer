...
LoadLanguage(forceLangCode := "") {
	...
	if (langID != "" && langID != "en") {
		...
		if !FileExist(langFile) {
			langFile := langDir StrSplit(langID, "-")[1] ".ini"
		}
		if FileExist(langFile) {
			LoadLangFile(langFile)
			currentLangCode := StrSplit(langID, "-")[1]
		}
	}
}
...
GetAvailableLanguages() {
	...
	Loop Files, langDir "*.ini" {
		SplitPath A_LoopFileName, , , , &code
		codeLower := StrLower(code)
		if codeNames.Has(codeLower) {
			result[code] := codeNames[codeLower]
		} else {
			baseCode := StrSplit(codeLower, "-")[1]
			result[code] := codeNames.Has(baseCode) ? codeNames[baseCode] : code
		}
	}
	return result
}
